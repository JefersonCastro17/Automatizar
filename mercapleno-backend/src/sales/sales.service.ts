...
export class SalesService {
  ...
  async createOrder(dto: CreateOrderDto, userId?: number) {
    ...
    try {
      ...
      // Verificar si alguno de los productos vendidos quedó con bajo stock y enviar alertas por correo
      try {
        const threshold = Number(process.env.LOW_STOCK_THRESHOLD) || 5;
        // Obtenemos los correos de administradores
        const [admins] = await this.db.query<any[]>(
          'SELECT email FROM usuarios WHERE id_rol = 1'
        );

        if (admins.length > 0) {
          for (const item of dto.items) {
            const idProducto = Number(item.id);
            const [[productRow]] = await this.db.query<any[]>(
              'SELECT p.nombre, s.stock FROM productos p JOIN stock_actual s ON p.id_productos = s.id_productos WHERE p.id_productos = ?',
              [idProducto],
            );
                ...
                for (const admin of admins) {
                  if (admin.email) {
                    try {
                      ...
                      console.error(`Error al enviar correo de bajo stock a ${admin.email}:`, emailError);
                    }
                  }
                }
              }
        ...
        // eslint-disable-next-line no-console
        console.error('Error al verificar stock tras venta:', checkError);
      }
